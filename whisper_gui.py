...
class WhisperTranscriber:
        ...
        try:
            ...
            audio.export(
                ...
                parameters=[
                    ...
                    "1",  # Mono
                    "-aq",
                    "4",  # Audio quality (4 is good for speech)
                ],
            )
    ...
    def split_audio(self, input_file, chunk_size_mb=20):
        ...
        if file_size <= chunk_size_mb * 1024 * 1024:
            print(
                f"File size is {file_size_mb:.2f} MB (under {chunk_size_mb}MB), no splitting needed"
            )
            return [input_file]
        ...
        print(
            f"File size is {file_size_mb:.2f} MB, splitting into {num_chunks} chunks..."
        )
        ...
        for i in range(num_chunks):
            ...
            temp_file = tempfile.NamedTemporaryFile(
                suffix=f"_chunk{i}.ogg", delete=False
            )
            chunk.export(
                temp_file.name,
                format="ogg",
        ...
        for part in srt_parts:
            ...
            for block in blocks:
                ...
                if len(lines) >= 3:  # At least number, timestamp, and text
                    ...
                    combined.append(
                        "\n".join(lines[2:])
                    )  # Text (handle multi-line subtitles)
...
class WhisperGUI:
    ...
    def setup_ui(self):
        ...
        ttk.Label(api_frame, text="Mode: OpenAI API", font=("Arial", 9, "bold")).pack(
            side=tk.LEFT
        )
        ...
        for lang_name, lang_code in common_langs:
            rb = ttk.Radiobutton(
                lang_frame, text=lang_name, variable=self.lang_var, value=lang_code
            )
        ...
        ttk.Label(
            self.root,
            text="Add important names, technical terms, or context to improve accuracy",
            font=("Arial", 8),
            foreground="gray",
        ).pack()
        ...
        ttk.Checkbutton(
            self.root, text="Create SRT subtitles", variable=self.create_srt
        ).pack(pady=5)
        ...
        self.status_var = tk.StringVar(
            value="Ready to transcribe! Click the 'Start Transcribing' button to begin."
        )
        ...
        # Secondary buttons
        ttk.Button(btn_frame, text="🗑️ Clear", command=self.clear_output).pack(
            side=tk.LEFT, padx=5, pady=5
        )

        ttk.Button(btn_frame, text="💾 Save Output", command=self.save_output).pack(
            side=tk.LEFT, padx=5, pady=5
        )
